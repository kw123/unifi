...
####
	...
	def startup(self):
		...
		#indigo.server.log(u"enableListener:"+ unicode(self.connectParams[u"enableListener"]))
		...
		if enableVideoSwitch:
			if self.isValidIP(ip0) and self.connectParams[u"UserID"]["unixNVR"] != "" and self.connectParams[u"PassWd"]["unixNVR"] != "":
...
####-------------------------------------------------------------------------####
	...
	def validateDeviceConfigUi(self, valuesDict, typeId, devId):
		try:
			if self.decideMyLog(u"Logic"): self.indiLOG.log(20,u"Validate Device dict:, devId:{}  vdict:{}".format(devId,valuesDict) )
	...
	#new for UDM (pro)
			...
			## video parameters
			if self.connectParams[u"UserID"]["unixNVR"]	!= valuesDict[u"nvrUNIXUserID"]:	  rebootRequired += u" nvrUNIXUserID changed;"
			if self.connectParams[u"PassWd"]["unixNVR"]	!= valuesDict[u"nvrUNIXPassWd"]:	  rebootRequired += u" nvrUNIXPassWd changed;"

			self.unifiVIDEONumerOfEvents	= int(valuesDict[u"unifiVIDEONumerOfEvents"])
			self.connectParams[u"UserID"]["unixNVR"]	= valuesDict[u"nvrUNIXUserID"]
			self.connectParams[u"PassWd"]["unixNVR"]	= valuesDict[u"nvrUNIXPassWd"]
			self.connectParams[u"UserID"]["nvrWeb"]	= valuesDict[u"nvrWebUserID"]
			self.connectParams[u"PassWd"]["nvrWeb"]	= valuesDict[u"nvrWebPassWd"]
			...
			if self.ipNumbersOf[u"VD"] != ip0 :
				...
				self.indiLOG.log(20,u"IP# old:{}, new:{}".format(self.ipNumbersOf[u"VD"], ip0) )
	...
	def printConfigMenu(self,  valuesDict=None, typeId=""):
		try:
			...
			self.myLog( text=u"WEB-UserID".ljust(40)						+	self.connectParams[u"UserID"]["webCTRL"] )
			self.myLog( text=u"WEB-PassWd".ljust(40)						+	self.connectParams[u"PassWd"]["webCTRL"] )
			...
			self.myLog( text=u"NVR-UNIXUserID".ljust(40)					+	self.connectParams[u"UserID"]["unixNVR"] )
			self.myLog( text=u"NVR-UNIXpasswd".ljust(40)					+	self.connectParams[u"PassWd"]["unixNVR"] )
			...
			self.myLog( text=u"NVR-WEB-UserID".ljust(40)					+	self.connectParams[u"UserID"]["nvrWeb"] )
			self.myLog( text=u"NVR-WEB-passWd".ljust(40)					+	self.connectParams[u"PassWd"]["nvrWeb"] )
	...
	def buttonRestartAPListenerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		if valuesDict[u"pickAP"] != "-1":
			self.restartRequest["APtail"] = valuesDict[u"pickAP"]
			self.restartRequest["APdict"] = valuesDict[u"pickAP"]
		return
	...
	def buttonSendCommandToNVRLEDCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		...
		valuesDict[u"retCodeCam"],x =  self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"],{"enableStatusLed":valuesDict[u"camLED"] == "1"})
		return valuesDict
	...
	def buttonSendCommandToNVRSoundsCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		self.addToMenuXML(valuesDict)
		valuesDict[u"retCodeCam"],x =  self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"],{"systemSoundsEnabled":valuesDict[u"camSounds"] == "1"} )
		return valuesDict
	...
	def buttonSendCommandToNVRenableSpeakerCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		self.addToMenuXML(valuesDict)
		valuesDict[u"retCodeCam"],x =  self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"],{"enableSpeaker":valuesDict[u"enableSpeaker"] == "1", "speakerVolume":int(valuesDict[u"enableSpeaker"])} )
		return valuesDict
	...
	def buttonSendCommandToNVRmicVolumeCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		self.addToMenuXML(valuesDict)
		valuesDict[u"retCodeCam"] = self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"],{"micVolume":int(valuesDict[u"micVolume"])} )
		return valuesDict
	...
	def buttonSendCommandToNVRRecordCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		...
		if valuesDict[u"postPaddingSecs"] =="-1" and valuesDict[u"prePaddingSecs"] =="-1":
			valuesDict[u"retCodeCam"],x =  self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"],
			...
			valuesDict[u"retCodeCam"]="bad selection for recording"
		return valuesDict
	...
	def buttonSendCommandToNVRIRCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		...
		if xxx.find(u"auto") >-1:
			valuesDict[u"retCodeCam"],x =  self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"],{"ispSettings":{"enableExternalIr": int(valuesDict[u"enableExternalIr"]),"irLedMode":"auto" }} )
		else:# for manual 0/100/255 level
			xxx = xxx.split("-")
			valuesDict[u"retCodeCam"],x =  self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"],{"ispSettings":{"enableExternalIr": int(valuesDict[u"enableExternalIr"]),"irLedMode":xxx[0], "irLedLevel": int(xxx[1])}  } )
		return valuesDict
	####-----------------	 ---------
	...
	def buttonSendCommandToNVRvideostreamingCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		...
		if u"channels" not in ret[0] or len(ret[0]["channels"]) !=3 : # something went wrong
			self.indiLOG.log(40,u"videostreaming error: "+error+ "     \n>>"+ unicode(ret)+"<<")
			valuesDict[u"retCodeCam"]=error
			return valuesDict

		# modify the required ones
		channels = ret[0]["channels"]
		channel	 = int(valuesDict[u"channelS"])
		channels[channel]["isRtmpEnabled"]	= valuesDict[u"isRtmpEnabled"]=="1"
		channels[channel]["isRtmpsEnabled"] = valuesDict[u"isRtmpsEnabled"]=="1"
		channels[channel]["isRtspEnabled"]	= valuesDict[u"isRtspEnabled"]=="1"
		# send back
		error, data=  self.setupNVRcmd(valuesDict[u"cameraDeviceSelected"], {"channels":channels}, cmdType=u"put")
		valuesDict[u"retCodeCam"]=error
		return valuesDict
	...
	def buttonSendCommandToNVRgetSnapshotCALLBACK(self, valuesDict=None, filter="", typeId="", devId="",returnCmd=False):
		self.addToMenuXML(valuesDict)
		if   self.imageSourceForSnapShot == "imageFromNVR": 	valuesDict[u"retCodeCam"] = self.getSnapshotfromNVR(valuesDict[u"cameraDeviceSelected"], valuesDict[u"widthOfImage"], valuesDict[u"fileNameOfImage"] )
		elif self.imageSourceForSnapShot == "imageFromCamera":	valuesDict[u"retCodeCam"] = self.getSnapshotfromCamera(valuesDict[u"cameraDeviceSelected"],                          valuesDict[u"fileNameOfImage"] )
		return valuesDict
	...
	def setupNVRcmd(self, devId, payload,cmdType=u"put"):
		...
		dev = indigo.devices[int(devId)]
		try:
			if not self.isValidIP(self.ipNumbersOf[u"VD"]): 		return "error IP",""
			if not self.VIDEOEnabled:					 	return "error enabled",""
			if not self.VIDEOEnabled:					 	return "error enabled",""
			if len(self.nvrVIDEOapiKey) < 5:				return "error apikey",""

			if payload !="":  payload['name']= dev.states[u"nameOnNVR"]
	...
	def executeCMDonNVR(self, data, cameraKey,	cmdType=u"put"):
		...
		try:
				...
				try:
					try:
						if time.time() - self.lastNVRCookie > 100: # re-login every 90 secs
							...
							if ret[1].find(u"error") >-1:
								self.indiLOG.log(40,u"error: (wrong UID/passwd, ip number?) ...>>"+ unicode(ret[0]) +u"<<\n"+unicode(ret[1])+u" Video error")
								return {}
					...
					try:
						...
						try:
							...
							self.indiLOG.log(20,u"UNIFI Video error  executeCMDonNVR has error, no json object returned: {} \n{}" + ret0, ret1)
							return []
						if u"rc" in jj["meta"] and unicode(jj["meta"]["rc"]).find(u"error")>-1:
							self.indiLOG.log(40,u"error: data:>>"+ ret0 +"<<\n>>"+ret1+"<<\n" +" UNIFI Video error")
							return []
			...
			#############does not work on OSX  el capitan ssl lib too old  ##########
				...
				if self.decideMyLog(u"Video"): self.indiLOG.log(20,u"Video executeCMDonNVR  cmdType: "+cmdType+u";  url: "+url +u";  dataDict: "+ dataDict+u"<<")
				try:
						...
						if u"rc" in jj[u"meta"] and unicode(jj[u"meta"][u"rc"]).find(u"error") >-1:
							self.indiLOG.log(40,u"executeCMDonNVR requests error: >>"+ unicode(resp.status_code) +"<<>>"+ response )
							return []
						elif self.decideMyLog(u"Video"):
							self.indiLOG.log(20,u"Video executeCMDonNVR requests "+response )
						return jj["data"]
				except	Exception, e:
	...
	def execVboxAction(self,action,action2=""):
		...
		if len(subprocess.Popen( testCMD, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True).communicate()[0]) > 10:
			try:   self.indiLOG.log(20,u"CameraInfo VBOXAction: still runing, not executing: {}  {}".format(action, action2) )
	...
	def buttonVboxActionStopCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		cmd = json.dumps({u"action":["stop"], u"vmMachine":self.vmMachine, u"vboxPath":self.vboxPath, u"logfile":self.logFile})
	...
	def buttonPrintTcpipStats(self):
		...
		for uType in sorted(self.dataStats[u"tcpip"].keys()):
			for ipNumber in sorted(self.dataStats[u"tcpip"][uType].keys()):
				if nSecs ==0:
					self.myLog( text=u"=== data stats for received messages ====     collection started at {}".format(time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(self.dataStats[u"tcpip"][uType][ipNumber]["startTime"]))), mType=u"data stats === START" )
				...
				nSecs = time.time() - self.dataStats[u"tcpip"][uType][ipNumber]["startTime"]
				nMin  = nSecs/60.
	...
	def getCamerasFromNVR(self,doPrint = False, action=[]):
		try:
			...
			#self.indiLOG.log(20," into getCamerasFromNVR action :{}".format(action))
			if u"system" in action:
				...
				if len(USERs)>0 and len(ACCOUNTs) >0:
					for account in ACCOUNTs:
						...
						if u"_id" in account and "username" in account and "name" in account:
							ID =  account["_id"]
							info["users"][ID] ={"userName":account["username"], "name":account[u"name"]}
							for user in USERs:
								##self.myLog( text="getCamerasFromNVR user dict: "+unicode(user))
								if u"accountId" in user and ID == user["accountId"]:
									##self.myLog( text="getCamerasFromNVR accountId ok and id found:"+ID)
									if u"apiKey" in user and "enableApiAccess" in user:
										...
										if u"enableApiAccess" in user and user["enableApiAccess"]: # its enabled, but no api key
											self.indiLOG.log(40,u"getCamerasFromNVR camera users   bad enableApiAccess / apiKey info for id:"+ unicode(ID)+"\n"+ unicode(USERs)+ " UNIFI error")
										else:
										...
										self.indiLOG.log(40,u"getCamerasFromNVR camera ACCOUNT bad _id / username / name info:\n"+ unicode(ACCOUNTs))

				server = self.getMongoData(cmdstr[0]+"server" +cmdstr[1])
				if len(server) >0:
					info["NVR"]		= server[0]

			#self.indiLOG.log(20," into getCamerasFromNVR info :{}".format(info))
			if u"cameras" in action:
				info["cameras"]	 = self.executeCMDonNVR( {}, "",  cmdType=u"get")
	...
	def printCameras(self, info):
		...
		try:
			if u"NVR" in info:
				self.myLog( text="--====================++++++++++++++++++++++++++++++++++++++++====================--",mType=u"System info-NVR:")
				for key in info["NVR"]:
					self.myLog( text=unicode(info["NVR"][key]),mType=u"  "+key )

				self.myLog( text="---====================++++++++++++++++++++++++++++++++++++++++====================--", mType=u"== System info- users:")
			if u"users" in info:
				nn = 0
						...
						if item in keepList or keepList == [u"*"]:
							if item == u"channels":
								...
								for nn in range(len(status[u"recordingStatus"])):
									out	 =	("motionRecordingEnabled: "+unicode(status[u"recordingStatus"][unicode(nn)][u"motionRecordingEnabled"])).ljust(30)
									out += "; fullTimeRecordingEnabled: "+unicode(status[u"recordingStatus"][unicode(nn)][u"fullTimeRecordingEnabled"])
	...
	def buttonConfirmSetWifiOptCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		for MAC in self.MAC2INDIGO[u"UN"]:
			try:
				dev = indigo.devices[self.MAC2INDIGO[u"UN"][MAC]["devId"]]
				props = dev.pluginProps
	...
	def buttonConfirmSetWifiUptimeCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		for MAC in self.MAC2INDIGO[u"UN"]:
			try:
				...
				props = dev.pluginProps
				if props[u"useWhatForStatus"].find(u"WiFi") > -1:
					props[u"useWhatForStatusWiFi"]	= "UpTime"
					dev.replacePluginPropsOnServer(props)
			except:
				pass
		self.printALLUNIFIsreduced()
	...
	def buttonConfirmSetNonWifiOptCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		for MAC in self.MAC2INDIGO[u"UN"]:
			try:
				...
				props = dev.pluginProps
				if props[u"useWhatForStatus"].find(u"WiFi") == -1:
					props[u"useWhatForStatus"]			= "OptDhcpSwitch"
	...
	def buttonConfirmSetNonWifiToSwitchCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		for MAC in self.MAC2INDIGO[u"UN"]:
			try:
				...
				props = dev.pluginProps
				if props[u"useWhatForStatus"].find(u"WiFi") == -1:
					props[u"useWhatForStatus"]			= "SWITCH"
					props[u"useupTimeforStatusSWITCH"]	= True
					dev.replacePluginPropsOnServer(props)
			except:
				pass
	...
	def filterWiFiDevice(self, filter="", valuesDict=None, typeId="", devId=""):
		...
		for dev in indigo.devices.iter(u"props.isUniFi"):
			...
			xList.append([dev.states[u"MAC"].lower(),dev.name+"--"+ dev.states[u"MAC"] +"-- AP:"+dev.states[u"AP"]])
	...
	def filterGroupMembers(self, valuesDict=None, filter="", typeId="", devId=""):
		...
		for mm in self.groupStatusList["Group"+unicode(sg)]["members"]:
			...
			xList.append([mm,dev.name + "- "+ dev.states[u"MAC"]])
	...
	def filterDevicesToAddToGroup(self, valuesDict=None, filter="", typeId="", devId=""):
		...
		for dev in indigo.devices.iter(u"props.isUniFi"):
			...
			xList.append([unicode(dev.id),dev.name + "- "+ dev.states[u"MAC"]])
		#self.myLog( text="group members: "+unicode(xList))
		return xList
...
######## block / unblock reconnect
	...
	def buttonConfirmReconnectCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		return self.executeCMDOnController(data={"cmd":"kick-sta","mac":valuesDict[u"selectedDevice"]},pageString=u"/cmd/stamgr",cmdType=u"post")
	...
	def buttonConfirmBlockCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		ret = self.executeCMDOnController(data={"cmd":"block-sta","mac":valuesDict[u"selectedDevice"]},pageString=u"/cmd/stamgr",cmdType=u"post")
	...
	def buttonConfirmUnBlockCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		ret = self.executeCMDOnController(data={"cmd":"unblock-sta","mac":valuesDict[u"selectedDevice"]}, pageString=u"/cmd/stamgr",cmdType=u"post")
		self.lastCheckForUDMUserReport = time.time() - self.unifigetBlockedClientsDeltaTime
		return ret
...
######## reports for specific stations / devices
	def buttonConfirmGetAPDevInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		for dev in indigo.devices.iter(u"props.isAP"):
			...
			if u"MAClan" in dev.states: 
				props = dev.pluginProps
				if u"useWhichMAC" in props and props[u"useWhichMAC"] == "MAClan":
					MAC = dev.states[u"MAClan"]
			self.indiLOG.log(20,u"unifi-Report getting _id for AP "+dev.name+ "  "+"/stat/device/"+MAC )
			jData= self.executeCMDOnController(data={}, pageString=u"/stat/device/"+MAC, jsonAction=u"returnData", cmdType=u"get")
			for dd in jData:
				if u"_id" not in dd:
					self.myLog( text="_id not in data  ", mType=u"unifi-Report")
					continue
		...
		for dev in indigo.devices.iter(u"props.isAP,props.isSwitch,props.isGateway"):
			...
			if u"MAClan" in dev.states and dev.states[u"MAClan"] != MAC:
				...
				if u"useWhichMAC" in props and props[u"useWhichMAC"] == "MAClan":
					MAC = dev.states[u"MAClan"]
			break	
		self.executeCMDOnController(data={}, pageString=u"/stat/device/"+MAC, jsonAction=u"print",startText=u"== Device print: /stat/device/"+MAC+" ==", cmdType=u"get")
		return
	...
	def buttonConfirmPrintClientInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		...
		self.executeCMDOnController(data={}, pageString=u"/stat/sta/"+MAC, jsonAction=u"print",startText=u"== Client print: /stat/sta/"+MAC+" ==", cmdType=u"get")
		return

######## reports all devcies
	####-----------------	 ---------
	def buttonConfirmPrintalluserInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		data = self.executeCMDOnController(data={}, pageString=u"/stat/alluser/", jsonAction=u"returnData", cmdType=u"get")
#		data = self.executeCMDOnController(data={"type":"all","conn":"all"}, pageString=u"/stat/alluser/", jsonAction=u"returnData", cmdType=u"get")
	...
	def buttonConfirmPrintuserInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId="", cmdType=u"get"):
		data = self.executeCMDOnController(data={}, pageString=u"/list/user/", jsonAction=u"returnData", cmdType=cmdType)
...
####   general reports
	...
	def buttonConfirmPrintHealthInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		data = self.executeCMDOnController(data={}, pageString=u"/stat/health/", jsonAction=u"returnData", cmdType=u"get")
		out = u"== HEALTH report ==\n"
		ii=0
		for item in data:
			ii+=1
			...
			if u"tx_bytes-r" in item:
				ll+=u"tx_B:"+(unicode(item["tx_bytes-r"])+";").ljust(9)
	...
	def buttonConfirmPrintPortForWardInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		data =self.executeCMDOnController(data={}, pageString=u"/stat/portforward/", jsonAction=u"returnData", cmdType=u"get")
		out = u"== PortForward report ==\n"
		out += u"##".ljust(4) + u"name".ljust(20) + u"protocol".ljust(10) + u"source".ljust(16)	+ u"fwd_port".ljust(9)+ u"dst_port".ljust(9)+ "fwd_ip".ljust(17)+ "rx_bytes".ljust(10)+ "rx_packets".ljust(17)+"\n"
		ii = 0
		for item in data:
			ii+=1
			...
			ll+= item[u"fwd_port"].ljust(9)
			ll+= item[u"dst_port"].ljust(9)
			ll+= item[u"fwd"].ljust(17)
			ll+= unicode(item[u"rx_bytes"]).ljust(10)
			ll+= unicode(item[u"rx_packets"]).ljust(10)
			out+=ll+("\n")
		self.indiLOG.log(20,u"unifi-Report ")
		self.indiLOG.log(20,u"unifi-Report  "+out)
		return
	...
	def buttonConfirmPrintSessionInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		...
		fromT 	= toT - 30000
		data = self.executeCMDOnController(data={}, pageString=u"/stat/session?type=all&start={:d}&end={:d}".format(fromT,toT), jsonAction=u"returnData", cmdType=u"get")
		out = u"\n"
		ii = 0
		for xxx in data:
			ii += 1
	...
	def buttonConfirmPrintAlarmInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		data = self.executeCMDOnController(data={}, pageString=u"/list/alarm/", jsonAction=u"returnData", cmdType=u"get")
	...
	def buttonConfirmPrintWifiConfigInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		data = self.executeCMDOnController(data={}, pageString=u"/rest/wlanconf", jsonAction=u"returnData", cmdType=u"get")
		out = "\n"
		ii = 0
		for xxx in data:
			ii += 1
	...
	def buttonConfirmPrintWifiChannelInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		data =self.executeCMDOnController(data={}, pageString=u"/stat/current-channel", jsonAction=u"returnData", cmdType=u"get")
	...
	def buttonConfirmPrintWlanConfInfoFromControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		data = self.executeCMDOnController(data={}, pageString=u"list/wlanconf", jsonAction=u"returnData", cmdType=u"get")
		out = u"==WLan Report =="+u"\n"
...
######## actions and menu set leds on /off
	...
	def buttonConfirmAPledONControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		self.executeCMDOnController(data={"led_enabled":True}, pageString=u"/set/setting/mgmt", cmdType=u"post")
		return
	...
	def buttonConfirmAPledOFFControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		self.executeCMDOnController(data={"led_enabled":False}, pageString=u"/set/setting/mgmt", cmdType=u"post")
		return
	...
	def buttonConfirmAPxledONControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		self.executeCMDOnController(data={"cmd":"set-locate","mac":valuesDict[u"selectedAPDevice"]}, pageString=u"/cmd/devmgr", cmdType=u"post")
		return valuesDict
	...
	def buttonConfirmAPxledOFFControllerCALLBACK(self, valuesDict=None, filter="", typeId="", devId=""):
		self.executeCMDOnController(data={"cmd":"unset-locate","mac":valuesDict[u"selectedAPDevice"]}, pageString=u"/cmd/devmgr", cmdType=u"post")
		return valuesDict
	...
	def execDisableAP(self, valuesDict,disable): #( true if disable )
		...
		self.executeCMDOnController(data={"disabled":disable}, pageString=u"/rest/device/"+ID, cmdType=u"put" ,cmdTypeForce=True)
		return valuesDict
...
######## set defaults for action and menu screens
	...
	def getActionConfigUiValues(self, pluginProps, typeId, devId):
		...
		if u"fileNameOfImage" in pluginProps:
			if len(self.changedImagePath) > 5:
				pluginProps["fileNameOfImage"] = self.changedImagePath+u"nameofCamera.jpeg"
			else:
				pluginProps["fileNameOfImage"] = self.indigoPreferencesPluginDir+u"nameofCamera.jpeg"
	...
	def addFirstSeenToStates(self):
		try:
			if self.unifiCloudKeyMode != u"ON": return
			listOfClients={}
			...
			for client in data:
				...
				listOfClients[client[u"mac"]] = {}
				if u"first_seen" in client:
					try: listOfClients[client[u"mac"]][u"first_seen"] = datetime.datetime.fromtimestamp(client[u"first_seen"]).strftime(u"%Y-%m-%d %H:%M:%S")
					except: pass

				if u"use_fixedip" in client:
						listOfClients[client[u"mac"]]["use_fixedip"] = client["use_fixedip"]
				else:
						listOfClients[client[u"mac"]]["use_fixedip"] = False

			for dev in indigo.devices.iter(u"props.isUniFi"):
				MAC = dev.states[u"MAC"]
				if	MAC in listOfClients:
					if u"first_seen" in listOfClients[MAC]:
						if u"firstSeen" in dev.states and dev.states[u"firstSeen"] != listOfClients[MAC][u"first_seen"]:
							dev.updateStateOnServer("firstSeen",listOfClients[MAC][u"first_seen"])

					if u"use_fixedip" in listOfClients[MAC]:
						if u"useFixedIP" in dev.states and dev.states[u"useFixedIP"] != listOfClients[MAC][u"use_fixedip"]:
							dev.updateStateOnServer(u"useFixedIP",listOfClients[MAC][u"use_fixedip"])
...
########  check if we have blocked/ unblocked devices
	...
	def checkForInforFromAllUsers(self, force= False):
		try:
			if self.unifiCloudKeyMode.find(u"ON") == -1 and not self.devsEnabled[u"UD"]: return
			if not force:
			...
			data =	  self.executeCMDOnController(data={}, pageString=u"stat/alluser", jsonAction=u"returnData", cmdType=u"get")
			if data == {}:
				self.indiLOG.log(20,u"No data returned from controller")#,mType=u"Connection")
				return
			for client in data:
				if len(client) ==0: continue
				...
				if self.unifigetBlockedClientsDeltaTime < 999:
					if u"blocked" in client:
						listOfBlockedClients[client[u"mac"]] = client[u"blocked"]

				if self.unifigetLastSeenDeltaTime < 999:
					listOfClients[client[u"mac"]] = {}
					if u"last_seen" in client:
						try: 	listOfClients[client[u"mac"]][u"last_seen"] = float(client[u"last_seen"])
						except: pass
				...
				if self.unifigetBlockedClientsDeltaTime < 999:
					if	MAC in listOfBlockedClients:
						if u"blocked" in dev.states:
							if dev.states[u"blocked"] != listOfBlockedClients[MAC]:
								dev.updateStateOnServer(u"blocked",listOfBlockedClients[MAC])
					else:
						if force:
							if u"blocked" in dev.states and dev.states[u"blocked"]:
								dev.updateStateOnServer(u"blocked",False)

				if self.unifigetLastSeenDeltaTime < 999:
					if	MAC in listOfClients:
						if u"last_seen" in listOfClients[MAC] and MAC in self.MAC2INDIGO[u"UN"]:
							lastUpTT   = self.MAC2INDIGO[u"UN"][MAC][u"lastUp"]
							if lastUpTT < listOfClients[MAC][u"last_seen"]: 
								self.MAC2INDIGO[u"UN"][MAC][u"lastUp"] = listOfClients[MAC]["last_seen"]
...
########  check if we have new unifi system devces, if yes: litt basic variables and request a reboot
	...
	def checkForNewUnifiSystemDevices(self):
		try:
			...
			deviceDict =		self.executeCMDOnController(data={}, pageString=u"/stat/device/", jsonAction=u"returnData", cmdType=u"get")
	...
	def executeMCAconfigDumpOnGW(self, valuesDict={},typeId="" ):
		...
		try:
			cmd = self.expectPath +" "
			cmd += "'"+self.pathToPlugin + self.connectParams[u"expectCmdFile"]["GWctrl"] + "' " 
			cmd += "'"+self.connectParams[u"UserID"][u"unixDevs"]+ "' '"+self.connectParams[u"PassWd"][u"unixDevs"]+ "' " 
			cmd += self.ipNumbersOf[u"GW"] + " " 
			cmd += "'"+self.connectParams[u"promptOnServer"][u"GWctrl"] + "' " 
			cmd += " XXXXsepXXXXX " + " " 
		...
		try:
				...
				try:
					if time.time() - self.lastUnifiCookieCurl > 100: # re-login every 90 secs
						...
						if (u"username" not in jj)   and   ( u"meta" not in jj or  jj[u"meta"][u"rc"] !=u"ok"):
							...
							return []
						if self.decideMyLog(u"ConnectionRET"):	 self.indiLOG.log(20,u"Connection-{}: {}".format(self.unifiCloudKeyIP,ret0) )
					...
					try:
						...
						if self.decideMyLog(u"ConnectionRET"):
								self.indiLOG.log(20,u"Connection to {}: returns >>{}<<".format(self.unifiCloudKeyIP, ret0) )
						elif self.decideMyLog(u"ConnectionCMD"):
								self.indiLOG.log(20,u"Connection to {}: returns >>{}...<<".format(self.unifiCloudKeyIP, ret0[0:100]) )
			...
			############# does not work on OSX	el capitan ssl lib too old	##########
				...
				try:
					...
					if unicode(e).find(u"CERTIFICATE_VERIFY_FAILED") > -1: 
						self.lastUnifiCookieRequests = 0
						return []
					self.indiLOG.log(40,u"in Line {} has error={}   Connection".format(sys.exc_traceback.tb_lineno, e) )
...
######## this for fixing the change from mac to MAC in states
		self.indiLOG.log(20,u"..getting vendor names for MAC#s")
		...
		self.indiLOG.log(20,u"..setup dicts  ..")
		delDEV = {}
		for dev in indigo.devices.iter(self.pluginId):
		...
		if self.VIDEOEnabled:
			...
			self.indiLOG.log(20,u"..setup NVR -3 saveCameraEventsStatus")
		...
		try:
			...
			if self.numberOfActive[u"AP"] > 0:
				self.indiLOG.log(20,u"..starting threads for {} APs,  {} sec apart (MSG-log and db-DICT)".format(self.numberOfActive[u"AP"],nsleep) )
				for ll in range(_GlobalConst_numberOfAP):
					if self.devsEnabled[u"AP"][ll]:
		...
		try:
			...
			if self.numberOfActive[u"SW"] > 0:
				...
				self.indiLOG.log(20,u"..starting threads for {} SWs {} sec apart (db-DICT only)".format(self.numberOfActive[u"SW"],nsleep) )
				for ll in range(_GlobalConst_numberOfSW):
					if self.devsEnabled[u"SW"][ll]:
						self.sleep(1.)
						if self.unifiControllerType.find(u"UDM") > -1 and ll == self.numberForUDM[u"SW"]: continue
						ipn = self.ipNumbersOf[u"SW"][ll]
						if self.decideMyLog(u"Logic"): self.indiLOG.log(20,u"START SW Thread tr # {}  uDM#:{}  {}".format(ll, self.numberForUDM[u"SW"], ipn, self.unifiControllerType))
	 ...
	 #					 self.trSWLog[unicode(ll)].start()
		...
		self.indiLOG.log(20,u"..starting threads finished" )
...
####-----------------   main loop            ---------
	...
	def doTheLoop(self):
		...
		if self.pendingCommand != []:
			if u"getCamerasFromNVR-print" in self.pendingCommand: self.getCamerasFromNVR(doPrint = True, action=["system","cameras"])
			if u"getCamerasIntoIndigo"	 in self.pendingCommand: self.getCamerasIntoIndigo(force = True)
			if u"getConfigFromNVR"		 in self.pendingCommand: self.getNVRIntoIndigo(force = True); self.getCamerasIntoIndigo(force = True)
	...
	def checkOnChanges(self):
		...
		try:
			...
			for devId in deldev:
				dd = deldev[devId]
				if self.decideMyLog(u"Logic", MAC=dd[0]): self.indiLOG.log(20,u"ChkOnChang del upDownTimers[{}],reason:{}".format(dd[0], dd[1]) )
					...
					if dev.deviceTypeId == u"UniFi":
						...
						if u"useWhatForStatus" in props:
							if props[u"useWhatForStatus"].find(u"WiFi") > -1:
								...
								elif  (u"useWhatForStatusWiFi" in props and props[u"useWhatForStatusWiFi"] == u"FastDown") and dev.states[u"status"] ==u"down" and (time.time() - lastUpTT > self.expTimeMultiplier * expT):
										if not self.expTimerSettingsOK("AP",MAC, dev): continue
										status = u"expired"
										changed = True
	...
	def expTimerSettingsOK(self,xType,MAC,	dev):
		try:
			...
			self.indiLOG.log(20,u"checking expiration timer settings {} updating exptime for {} to {} as it is shorter than reading dicts: {}+10".format(mac, dev.name, newExptime, self.readDictEverySeconds[xType]))
	...
	def testAPandPing(self,ipNumber, cType):
		try:
			if self.decideMyLog(u"Expect"): self.indiLOG.log(20,u"CONNtest  testing if {} {} {} is running ".format(ipNumber, self.expectPath,self.connectParams[u"expectCmdFile"][cType]))
			if os.path.isfile(self.pathToPlugin +self.connectParams[u"expectCmdFile"][cType]):
				if self.decideMyLog(u"Expect"): self.indiLOG.log(20,u"CONNtest {} exists, now doing ping" .format(self.connectParams[u"expectCmdFile"][cType]))
	...
	def addTypeToDataStats(self,ipNumber, apN, uType):
		try:
			...
			if ipNumber not in self.dataStats[u"tcpip"][uType]:
				self.dataStats[u"tcpip"][uType][ipNumber]={u"inMessageCount":0,u"inMessageBytes":0,u"inErrorCount":0,u"restarts":0,u"startTime":time.time(),u"APN":unicode(apN), u"aliveTestCount":0}
		except	Exception, e:
			self.indiLOG.log(40,u"in Line {} has error={}".format(sys.exc_traceback.tb_lineno, e))
	####-----------------	 ---------
		...
		try:
			...
			for cam2 in camJson:
				if u"mac" in cam2:
					...
					if not found:
						saveCam = True
						self.cameras[MAC]= {"nameOnNVR":cam2[u"name"], "events":{}, "eventsLast":{"start":0,"stop":0},"devid":-1, "uuid":cam2["uuid"], "ip":cam2["host"], "apiKey":cam2["_id"]}
		...
		try:
			...
			while self.pluginState != "stop":
				...
				try:
					...
					#eventLogList 		= self.executeCMDOnController(data={"_sort":"+time", "_limit":min(500,max(10,nrec))}, pageString=u"/stat/event/", jsonAction=u"returnData", cmdType=u"post")
					eventLogList 		= self.executeCMDOnController(data={}, pageString=u"/stat/event/", jsonAction=u"returnData", cmdType=u"get") # UDM pro: post does not work, so we need to get all data
					thisRecIds			= []
					...
					if not lastRecIdFound and lastRecIds !=[]:
						eventLogList 		= self.executeCMDOnController(data={"_sort":"+time", "_limit":min(500,max(10,nrec*3))}, pageString=u"/stat/event/", jsonAction=u"returnData", cmdType=u"post")
						thisRecIds			= []
						for logEntry in eventLogList:
							thisRecIds.append(logEntry["_id"])
					...
					for logEntry in eventLogList[::-1]:
						...
						if u"ap" in logEntry: 
							...
							if self.MAC2INDIGO["AP"][MAC_AP_Active]["ipNumber"] =="":
								...
								continue
							#if self.MAC2INDIGO["AP"][MAC_AP_Active]["ipNumber"] != self.ipNumbersOf[u"AP"][self.numberForUDM[u"AP"]]: continue  # ignore non UDM log entries 
						...
						if MAC_AP_Active == "":
							if MAC in self.MAC2INDIGO[u"UN"]:
								for MACap in self.MAC2INDIGO["AP"]:
									if int(self.MAC2INDIGO["AP"][MACap]["apNo"]) == int(self.numberForUDM[u"AP"]):
										MAC_AP_Active = MACap
										break
	...
	def createAPdeviceIfNeededForUDM(self, MAC, line, fromTo):
		...
		xType		= "AP"
		isType 		= "isAP"
		if u"radio"+fromTo in line: radio = line["radio"+fromTo]
		if u"essid"+fromTo in line: essid = line["ssid"+fromTo]
			...
			while True:
				...
				if ( (time.time()- lastForcedRestartTimeStamp) > minWaitbeforeRestart) or lastForcedRestartTimeStamp <0: # init comm
							...
							if self.decideMyLog(u"Expect"):
								...
								self.indiLOG.log(20,u"getMessages: ListenProcess started for uType: {};  ip: {}, prompt:'{}', pid:{}".format(uType, ipNumber, self.connectParams[u"promptOnServer"][ipNumber], pid) )
				...
				######### for tail logfile
					...
					######### for Dicts
						...
						#if self.decideMyLog(u"Special"): self.indiLOG.log(20,u"linesFromServer  uType:{};  splitting:startDict:{};  ppp:{},  {} .... {}".format(uType, self.connectParams[u"startDictToken"][uType], len(ppp), total[0:100],total[-100:]  ) )

						if len(ppp) == 2:
							#if self.decideMyLog(u"Special"): self.indiLOG.log(20,u"linesFromServer   found endDictToken:{}, ? {} ".format( self.connectParams[u"endDictToken"][uType], ppp[1].find(self.connectParams[u"endDictToken"][uType]) ) )
							if ppp[1].find(self.connectParams[u"endDictToken"][uType]) >-1:
								...
								try:
									...
									if unicode(e) != u"None":
										...
										self.indiLOG.log(40,u"... {}  {} error in the JSON data, #of bytes:{}".format(uType, ipNumber,len(total)))
										self.indiLOG.log(40,u"... JSON={}...\n...  {}".format(msgF[0:100], msgF[-40:]) )
...
#1561518324.741 2019-06-25 22:05:24.741/CDT: INFO   [uv.analytics.motion] [AnalyticsService] [FCECDA1F1532|LivingRoom-Window-Flex] MotionEvent type:start event:1049 clock:111842854 in AnalyticsEvtBus-0
				...
				if dev.states[u"eventNumber"] > evNo or ( self.cameras[MAC]["events"][evNo][evType] <= self.cameras[MAC]["eventsLast"][evType]) :
					try:
		...
		try:
			...
			for line in lines:
				...
				if webApiLog: # message from UDM re-packaged as AP message
					...
					up = True
					token = "steady"
					if line["key"].lower().find(u"disconnected") >-1:
						token = "DISCONNECTED"
						up = False
					if line["key"].lower().find(u"disassociated") >-1:
						token = "DISCONNECTED"
						up = False

					#### roaming:::::
					elif line["key"].lower().find(u"roam") >-1:
					...
					GHz = "2"
					if u"channel"    in line and int(line["channel"])    >= 12:	GHz = "5"
					if u"channel_to" in line and int(line["channel_to"]) >= 12:	GHz = "5"
					timeOfMSG = line["time"]
					...
					if not new:
						...
						if u"useWhatForStatus" in props and props[u"useWhatForStatus"].find(u"WiFi") > -1:
							...
							if up: # is up now
								...
								if self.decideMyLog(u"Logic", MAC=MAC): self.indiLOG.log(20,u"MS-AP-WF-R   ==> restart exptimer use AP log-msg, exp-time left:{:5.1f}".format(self.getexpT(props) -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"]) ))
								self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
								...
								try:
									...
									if ipNumberAP == "" or ipNumberAP == oldIP.split(u"-")[0]: # only if its on the same current AP
										...
										if u"useWhatForStatusWiFi" in props and props[u"useWhatForStatusWiFi"] in ["FastDown","Optimized"]:
											if self.decideMyLog(u"LogDetails", MAC=MAC): self.indiLOG.log(20,u"MS-AP-WF-5  .. checking timer,   token:down;  tt-uptDelay:{:4.1f}".format(dt) )
											...
											if self.decideMyLog(u"LogDetails", MAC=MAC): self.indiLOG.log(20,u"MS-AP-WF-6  .. ipNumberAP:{} 'delay settings updown timer < 2; set uptimer =0, downtimer =tt'".format( ipNumberAP))
...
###					self.indiLOG.log(20,u"DICTDATA    rejected .. mac, vap_table,  ip not in dict ..{}".format(apDict))
	...
	def doSWITCHdictClients(self, apDict, swNumb, ipNDevice, MACSW, hostnameSW, ipNumber):
		...
		try:
			...
			for port in portTable:
				...
				if u"is_uplink"	  in port and port["is_uplink"]:			isUpLink   = True
				elif u"lldp_table" in port and len(port["lldp_table"]) > 0:	isDownLink = True
			...
			if len(host_table) > 0:
				for item in host_table:
					...
					if not new:
							...
							newStatus = "up"
							if u"useWhatForStatus" in props and props[u"useWhatForStatus"] in ["DHCP","OptDhcpSwitch","WiFi,DHCP"]:
								...
								if u"useAgeforStatusDHCP" in props and props[u"useAgeforStatusDHCP"] != "-1"     and float(age) > float( props[u"useAgeforStatusDHCP"]):
										if dev.states[u"status"] == "up":
											if u"usePingDOWN" in props and props[u"usePingDOWN"] and self.sendWakewOnLanAndPing(MAC,dev.states[u"ipNumber"], props=props, calledFrom ="doGWHost_table") == 0:  # did  answer
												if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"    {} restart exptimer DICT network_table AGE>max:{}, but answers ping, exp-time left:{:5.1f}".format(MAC, props[u"useAgeforStatusDHCP"], self.getexpT(props) -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"]) ))
												self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
												newStatus = "up"
											else:
									...
									if u"usePingUP" in props and props[u"usePingUP"] and dev.states[u"status"] =="up" and self.sendWakewOnLanAndPing(MAC,dev.states[u"ipNumber"], props=props, calledFrom ="doGWHost_table") == 1:	# did not answer
					...
					if not new:
							...
							if u"useWhatForStatus" in props and props[u"useWhatForStatus"] in ["DHCP","OptDhcpSwitch","WiFi,DHCP"]:
								...
								if u"useAgeforStatusDHCP" in props and props[u"useAgeforStatusDHCP"] != "-1"     and float(age) > float( props[u"useAgeforStatusDHCP"]):
										if dev.states[u"status"] == "up":
											if u"usePingDOWN" in props and props[u"usePingDOWN"] and self.sendWakewOnLanAndPing(MAC,dev.states[u"ipNumber"], props=props, calledFrom ="doGWHost_table") == 0:  # did  answer
												if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-DPI-2   {} ==> restart exptimer DICT network_table AGE>max, but answers ping {}, exp-time left:{:5.1f}".format(MAC,props[u"useAgeforStatusDHCP"], self.getexpT(props) -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])))
												self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
												newStatus = "up"
											else:
							...
							if u"useWhatForStatus" in props and props[u"useWhatForStatus"].find(u"WiFi") > -1:
								...
								if u"useWhatForStatusWiFi" not in props or ("useWhatForStatusWiFi" in props and	props[u"useWhatForStatusWiFi"] !="FastDown"):
									...
									if u"useWhatForStatusWiFi" in props and ( props[u"useWhatForStatusWiFi"] == "Optimized"):
										if self.decideMyLog(u"DictDetails", MAC=MAC) or self.decideMyLog(u"Logic", MAC=MAC): self.indiLOG.log(20,u"DC-WF-o1   {:15s}  {}; .. using optimized for status; idle uptimes {} < {} or uptime (new){} != {}(Old)" .format(ipNumber, MAC, idletime, idleTimeMaxSecs, newUpTime, oldUpTime))

										if oldStatus == "up":
											if (  float(newUpTime) != float(oldUpTime)	) or  ( float(idletime)  < idleTimeMaxSecs ):
													if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-o2R  {:15s}  {}; ==> restart exptimer use idleTime, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])))
													self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
											else:
												if ( oldAssociated.split("-")[0] != newAssociated ): # ignore new AP
													if self.decideMyLog(u"Logic", MAC=MAC): self.indiLOG.log(20,u"DC-WF-o3R  {:15s}  {}; ==> restart exptimer, associated to new AP:{} from:{}, exp-time left:{:5.1f}".format(ipNumber, MAC, oldAssociated, newAssociated, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])) )
													self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
												else: # same old
													if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-o4DN {:15s}  {}; set timer to expire in 10 secs use idleTime/uptime".format(ipNumber, MAC))
													self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()- expTime + 10

										else: # old = down
											if ( float(newUpTime) != float(oldUpTime) ) or (  float(idletime) <= idleTimeMaxSecs ):
												if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-o5   {:15s}  {}; status Down->up; ==> restart exptimer, use idleTime/uptime, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"]) ))
												self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
												self.setImageAndStatus(dev, "up",oldStatus=oldStatus,ts=time.time(),reason=u"DICT "+suffixN+u" "+ipNumber+u" up Optimized")
											else:
												if ( oldAssociated.split("-")[0] != newAssociated ): # ignore new AP
													if self.decideMyLog(u"Logic", MAC=MAC): self.indiLOG.log(20,u"DC-WF-o6R  {:15s}  {}; ==> restart exptimer, status up new AP; use idleTime/uptime, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])))
													self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()


									elif u"useWhatForStatusWiFi" in props and (props[u"useWhatForStatusWiFi"] =="IdleTime" ):
										if self.decideMyLog(u"DictDetails", MAC=MAC) or self.decideMyLog(u"Logic", MAC=MAC): self.indiLOG.log(20,u"DC-WF-i0-  {:15s}  {};.  IdleTime..  checking IdleTime {} < {}  old/new associated {}/{}".format(ipNumber, MAC,idletime, idleTimeMaxSecs, oldAssociated.split("-")[0], newAssociated)) 
										...
										if float(idletime)	> idleTimeMaxSecs and oldStatus == "up":
											if ( oldAssociated.split("-")[0] == newAssociated ):
												if u"usePingDOWN" in props and props[u"usePingDOWN"] and self.sendWakewOnLanAndPing(MAC,dev.states[u"ipNumber"], props=props, calledFrom ="doWiFiCLIENTSdict") ==0:
														if self.decideMyLog(u"DictDetails"): self.indiLOG.log(20,u"DC-WF-i1R  {:15s}  {}; reset exptimer, ping was test up, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])) )
														self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
												else:
													if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-i2DN {:15s}  {}; status down in 10 secs".format(ipNumber, MAC))
													self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()- expTime + 10
											else:
												if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-i3R  {:15s}  {}; status up new AP use IdleTime, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])))
												self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()

										elif float(idletime)  <= idleTimeMaxSecs:
											if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-i4R  {:15s}  {}; ==> restart exptimer, use IdleTime, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])))
											self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
											if oldStatus != "up":
												self.setImageAndStatus(dev, "up",oldStatus=oldStatus,ts=time.time(),reason=u"DICT "+ipNumber+u" "+suffixN+u" up IdleTime")
												if self.decideMyLog(u"Logic", MAC=MAC): self.indiLOG.log(20,u"DC-WF-i5R  {:15s}  {}; status up, use IdleTime".format(ipNumber, MAC))
										...
										if newUpTime == oldUpTime and oldStatus == "up":
											if u"usePingUP" in props and props[u"usePingUP"] and self.sendWakewOnLanAndPing(MAC,dev.states[u"ipNumber"], props=props, calledFrom ="doWiFiCLIENTSdict") == 0:
													if self.decideMyLog(u"DictDetails", MAC=MAC):self.indiLOG.log(20,u"DC-WF-u2   {:15s}   {}; ==> restart exptimer, ping test ok, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])) )
													self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
											else:
												if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-u3DN {:15s}  {}; let timer expire, Uptime is not changed".format(ipNumber, MAC) )
											...
											if self.decideMyLog(u"DictDetails", MAC=MAC): self.indiLOG.log(20,u"DC-WF-u4   {:15s}  {}; ==> restart exptimer, normal extension, exp-time left:{:5.1f}".format(ipNumber, MAC, expTime -(time.time()-self.MAC2INDIGO[xType][MAC][u"lastUp"])))
											self.MAC2INDIGO[xType][MAC][u"lastUp"] = time.time()
	...
	def doGatewaydictSELF(self, gwDict, ipNumber):
		...
		try:
			...
			if self.decideMyLog(u"UDM"):  self.indiLOG.log(20,u"doGw     unifiControllerType:{}; if.. find UDM >-1:{}".format(self.unifiControllerType, self.unifiControllerType.find(u"UDM") > -1) )
			...
			if u"up" in wan:										wanUP  = wan["up"]
			if u"up" in wan2:									wan2UP = wan2["up"]
			...
			if u"system-stats" in gwDict:
				...
				if u"cpu" in sysStats:	 cpuPercent = sysStats["cpu"]
				if u"mem" in sysStats:	 memPercent = sysStats["mem"]
				for xx in ["temps"]:
					if xx in sysStats:
						...
						if len(gwDict[xx]) > 0:
							if type(gwDict[xx]) == type([]):
								try:
									for yy in gwDict[xx]:
										if u"name" in yy: 
											if yy[u"name"] == "Local": 	temperature		 		= GT.getNumber(yy["value"])
											if yy[u"name"] == "PHY":  	temperature_Board_PHY 	= GT.getNumber(yy["value"])
											if yy[u"name"] == "CPU": 	temperature_Board_CPU 	= GT.getNumber(yy["value"])
									#self.myLog( text="doGatewaydictSELF sysStats[temp]ok : "+temperature)
								except:
			...
			if u"speedtest_ping" in wan:		wanPingTime		= "%4.1f" % wan[u"speedtest_ping"] + u"[ms]"
			if u"latency" in wan:			wanLatency		= "%4.1f" % wan[u"latency"] + u"[ms]"
			if u"xput_down" in wan:			wanDownload		= "%4.1f" % wan[u"xput_down"] + u"[Mb/S]"
			if u"xput_up" in wan:			wanUpload		= "%4.1f" % wan[u"xput_up"] + u"[Mb/S]"

			if u"speedtest_ping" in wan2:	wan2PingTime	= "%4.1f" % wan2[u"speedtest_ping"] + u"[ms]"
			if u"latency" in wan2:			wan2Latency		= "%4.1f" % wan2[u"latency"] + u"[ms]"
			if u"xput_down" in wan2:			wan2Download	= "%4.1f" % wan2[u"xput_down"] + u"[Mb/S]"
			if u"xput_up" in wan2:			wan2Upload		= "%4.1f" % wan2[u"xput_up"] + u"[Mb/S]"
			...
			if MAC in self.MAC2INDIGO[u"UN"]:
				...
				try:
					...
					if devUN.states[u"status"] !=u"up":
						...
						if self.decideMyLog(u"Logic", MAC=MAC) :  self.indiLOG.log(20,u"updateself setStatusUpForSelfUnifiDev:      updating status to up MAC:" + MAC+"  "+devUN.name+"  was: "+ devUN.states[u"status"] )
	...
	def setImageAndStatus(self, dev, newStatus, oldStatus=u"123abc123abcxxx", ts="", level=1, text1="", iType=u"", force=False, fing=True,reason=u""):
		try:
			...
			if  self.decideMyLog(u"", MAC=MAC): self.indiLOG.log(20,u"STAT-Chang {} data in: newSt:{}; oldStIn:{}; oldDevSt:{}".format(MAC, newStatus, oldStatus, dev.states[u"status"]))
			if oldStatus == u"123abc123abc":
				oldStatus = dev.states[u"status"]

			try:	xType = self.xTypeMac[unicode(dev.id)]["xType"]
			except: 
				self.indiLOG.log(20,u"STAT-Chang error for devId:{} xType bad:{}".format(dev.id, self.xTypeMac[unicode(dev.id)]))
				return
	...
	def exeDisplayStatus(self, dev, status, force=True):
		...
		if force or status =="":
			...
			dev.updateStateOnServer(u"onOffState",value= dev.states[u"status"] in ["up","rec","ON"], uiValue= dev.states[u"displayStatus"])
		return
